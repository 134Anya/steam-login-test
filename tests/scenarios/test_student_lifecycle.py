...
class TestStudentLifecycle:
    def test_student_grade_count(self, university_api_utils_admin, temp_teacher, temp_student):
        service = UniversityService(api_utils=university_api_utils_admin)
        student_id = temp_student.id

        for val in [5, 4, 3]:
            service.create_grade(GradeRequest(student_id=student_id, teacher_id=temp_teacher.id, grade=val))
